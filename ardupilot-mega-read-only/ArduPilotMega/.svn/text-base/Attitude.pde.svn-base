...
void calc_throttle()
{
	...
	g.channel_throttle.servo_out += (g.channel_pitch.servo_out * g.kff_pitch_compensation);
...
void set_servos_4(void)
{
	if(control_mode == MANUAL){
		...
		if (mix_mode == 0){
			...
			g.channel_roll.radio_out =	elevon1_trim + (reverse_ch1_elevon * (ch1 * 500.0/ g.roll_servo_max));
			g.channel_pitch.radio_out =	elevon2_trim + (reverse_ch2_elevon * (ch2 * 500.0/ g.pitch_servo_max));
		}
